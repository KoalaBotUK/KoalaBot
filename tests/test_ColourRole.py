...
async def test_list_protected_roles(num_total, num_protected):
    ...
    await dpytest.message(KoalaBot.COMMAND_PREFIX[0] + "list_protected_role_colours")
...
async def test_list_custom_colour_allowed_roles(num_total, num_protected):
    ...
    await dpytest.message(KoalaBot.COMMAND_PREFIX[0] + "list_custom_colour_allowed_roles")
    msg: discord.Message = await dpytest.sent_queue.get()
    assert expected in msg.content
    for r in allowed:
        assert r.mention in msg.content, r.mention + " " + msg.content
...
async def test_add_protected_role_colour():
    ...
    assert independent_get_protected_colours(guild.id) == []
    await dpytest.message(KoalaBot.COMMAND_PREFIX[0] + "add_protected_role_colour " + str(role[0].id))
...
async def test_add_custom_colour_allowed_role():
    ...
    role = await make_list_of_roles(guild, 1)
    assert independent_get_colour_change_roles(guild.id) == []
    await dpytest.message(KoalaBot.COMMAND_PREFIX[0] + "add_custom_colour_allowed_role " + str(role[0].id))
...
async def test_remove_protected_role_colour():
    ...
    await dpytest.message(KoalaBot.COMMAND_PREFIX[0] + "remove_protected_role_colour " + str(role.id))
    assert independent_get_protected_colours(guild.id) == []
...
async def test_remove_custom_colour_allowed_role():
    ...
    await dpytest.message(KoalaBot.COMMAND_PREFIX[0] + "remove_custom_colour_allowed_role " + str(role.id))
    assert independent_get_colour_change_roles(guild.id) == []
...
async def test_custom_colour_check_failure():
    ...
    with pytest.raises(commands.CheckFailure):
        await dpytest.message(KoalaBot.COMMAND_PREFIX[0] + "custom_colour ab1234")
        assert dpytest.verify().message().content("You don't have the required role to use this command.")
        assert dpytest.verify().message().nothing()
    with pytest.raises(commands.CheckFailure):
        await dpytest.message(KoalaBot.COMMAND_PREFIX[0] + "custom_colour no")
        assert dpytest.verify().message().content("You don't have the required role to use this command.")
        assert dpytest.verify().message().nothing()
...
async def test_custom_colour_no_no_colour_role():
    ...
    await member.add_roles(role)
    await dpytest.message(KoalaBot.COMMAND_PREFIX[0] + "custom_colour no", member=0)
...
async def test_custom_colour_colour_is_protected():
    ...
    await dpytest.message(KoalaBot.COMMAND_PREFIX[0] + "custom_colour FEFEFE", member=0)
...
async def test_custom_colour_invalid_colour_str():
    ...
    member: discord.Member = dpytest.get_config().members[0]
    await member.add_roles(role)
    await dpytest.message(KoalaBot.COMMAND_PREFIX[0] + "custom_colour s34a21", member=0)
    assert dpytest.verify().message().content(
...
async def test_custom_colour_valid():
    ...
    member: discord.Member = dpytest.get_config().members[0]
    await member.add_roles(role)
    await dpytest.message(KoalaBot.COMMAND_PREFIX[0] + "custom_colour e34a21", member=0)
