...
async def test_add_user_to_twitch_alert(twitch_cog):
    ...
    await dpytest.message(
        f"{KoalaBot.COMMAND_PREFIX}twitch add {dpytest.get_config().channels[0].id} monstercat")
    assert dpytest.verify().message().embed(embed=assert_embed)
...
async def test_add_user_to_twitch_alert_custom_message(twitch_cog):
    ...
    await dpytest.message(
        f"{KoalaBot.COMMAND_PREFIX}twitch add {channel.id} monstercat {test_custom_message}", channel=-1,
        member=member)
    assert dpytest.verify().message().embed(embed=assert_embed)
...
async def test_remove_user_from_twitch_alert_with_message(twitch_cog):
    ...
    await dpytest.message(f"{KoalaBot.COMMAND_PREFIX}twitch remove {channel.id} monstercat", channel=-1,
                          member=member)
...
async def test_remove_user_from_twitch_alert_wrong_guild(twitch_cog):
    ...
    await dpytest.message(
        f"{KoalaBot.COMMAND_PREFIX}twitch remove {dpytest.get_config().channels[0].id} monstercat",
        channel=-1, member=member)
    assert dpytest.verify().message().embed(
        embed=TwitchAlert.error_embed("The channel ID provided is either invalid, or not in this server."))
...
async def test_add_team_to_twitch_alert(twitch_cog):
    ...
    assert_embed = discord.Embed(title="Added Team to Twitch Alert",
                                 ...
                                 colour=KOALA_GREEN)
    # Creates Twitch Alert
    await dpytest.message(f"{KoalaBot.COMMAND_PREFIX}twitch addTeam {channel.id} faze", channel=-1,
                          member=member)
...
async def test_add_team_to_twitch_alert_with_message(twitch_cog):
    ...
    assert_embed = discord.Embed(title="Added Team to Twitch Alert",
                                 ...
                                 colour=KOALA_GREEN)
    # Creates Twitch Alert
    await dpytest.message(f"{KoalaBot.COMMAND_PREFIX}twitch addTeam {channel.id} faze wooo message",
                          channel=-1, member=member)
    assert dpytest.verify().message().embed(assert_embed)
...
async def test_add_team_to_twitch_alert_wrong_guild(twitch_cog):
    ...
    # Creates Twitch Alert
    await dpytest.message(
        f"{KoalaBot.COMMAND_PREFIX}twitch addTeam {dpytest.get_config().channels[0].id} faze ", channel=-1,
        member=member)
    assert dpytest.verify().message().embed(
        embed=TwitchAlert.error_embed("The channel ID provided is either invalid, or not in this server."))
...
async def test_remove_team_from_twitch_alert_with_message(twitch_cog):
    ...
    # Creates Twitch Alert
    await dpytest.message(f"{KoalaBot.COMMAND_PREFIX}twitch addTeam {channel.id} faze {test_custom_message}",
                          channel=-1, member=member)
    await dpytest.empty_queue()
    # Removes Twitch Alert
    await dpytest.message(f"{KoalaBot.COMMAND_PREFIX}twitch removeTeam {channel.id} faze", channel=-1,
                          member=member)
...
async def test_remove_team_from_twitch_alert_wrong_guild(twitch_cog):
    ...
    await dpytest.message(
        f"{KoalaBot.COMMAND_PREFIX}twitch removeTeam {dpytest.get_config().channels[0].id} monstercat",
        channel=-1, member=member)
    assert dpytest.verify().message().embed(
        embed=TwitchAlert.error_embed("The channel ID provided is either invalid, or not in this server."))
...
async def test_loop_check_live(twitch_cog):
    ...
    await dpytest.message(f"{KoalaBot.COMMAND_PREFIX}twitch add 7363 monstercat")
    await dpytest.empty_queue()
...
def twitch_alert_db_manager(twitch_cog):
    return TwitchAlert.TwitchAlertDBManager(twitch_cog.bot)
...
def test_create_tables(twitch_alert_db_manager):
    parent_database_manager = twitch_alert_db_manager
    twitch_alert_db_manager.create_tables()
...
def test_new_ta(twitch_alert_db_manager_tables):
    ...
    assert twitch_alert_db_manager_tables.db_execute_select(sql_check_db_updated) == \
...
def test_add_user_to_ta_default_message(twitch_alert_db_manager_tables):
    ...
    parent_database_manager = twitch_alert_db_manager_tables
...
def test_add_team_to_ta(twitch_alert_db_manager_tables):
    ...
    assert twitch_alert_db_manager_tables.db_execute_select(sql_select_team) == [(None,)]
...
def test_add_team_to_ta_custom_message(twitch_alert_db_manager_tables, channel_id=573, guild_id=574):
    ...
    assert twitch_alert_db_manager_tables.db_execute_select(sql_select_team) == \
           [("Message here",)]
...
async def test_remove_team_from_ta_deletes_messages(twitch_alert_db_manager_tables):
    ...
    twitch_alert_db_manager_tables.db_execute_commit(sql_add_message)
...
async def test_update_team_members(twitch_alert_db_manager_tables):
    ...
    twitch_alert_db_manager_tables.db_execute_commit(sql_insert_monstercat_team)
    ...
    sql_select_monstercat_team = "SELECT twitch_username " \
                                 ...
                                 "WHERE team_twitch_alert_id = 604 AND twitch_username='monstercat'"
    result = twitch_alert_db_manager_tables.db_execute_select(sql_select_monstercat_team)
    assert result != []
    pass
...
async def test_update_all_teams_members(twitch_alert_db_manager_tables):
    ...
    result = twitch_alert_db_manager_tables.db_execute_select(sql_select_monstercats_team)
    assert len(result) == 2
    pass
...
async def test_delete_all_offline_streams(twitch_alert_db_manager_tables, bot: discord.ext.commands.Bot):
    ...
    result = twitch_alert_db_manager_tables.db_execute_select(sql_select_messages)
    assert len(result) == 1
