...
def cleanup(guildId, tf_cog):
    tf_cog.tf_database_manager.database_manager.db_execute_commit(
        f"DELETE FROM TextFilter WHERE guild_id=(\"{guildId}\");")
...
async def test_filter_new_word_correct_database(tf_cog):
    ...
    assert len(tf_cog.tf_database_manager.database_manager.db_execute_select(
        f"SELECT filtered_text FROM TextFilter WHERE filtered_text = 'no';")) == old + 1
...
def text_filter_db_manager():
    KoalaBot.database_manager.insert_setup_status(dpytest.get_config().guilds[0].id)
    KoalaBot.database_manager.update_guild_setup_status(dpytest.get_config().guilds[0].id)
    return TextFilter.TextFilterDBManager(KoalaDBManager.KoalaDBManager(KoalaBot.DATABASE_PATH, KoalaBot.DB_KEY),
                                          dpytest.get_config())
...
async def test_add_mod_channel_tag(text_filter_db_manager, tf_cog):
    ...
    result = text_filter_db_manager.database_manager.db_execute_select(
        "SELECT channel_id FROM TextFilterModeration WHERE guild_id = ?;", args=[channel.guild.id])
