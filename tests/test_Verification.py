...
def setup_function():
    ...
    db_manager.db_execute_commit("DROP TABLE VerifiedEmails")
    db_manager.db_execute_commit("DROP TABLE NonVerifiedEmails")
    db_manager.db_execute_commit("DROP TABLE ToReVerify")
    db_manager.db_execute_commit("DROP TABLE Roles")
    db_manager.db_execute_commit("CREATE TABLE VerifiedEmails (user_id, email)")
    db_manager.db_execute_commit("CREATE TABLE NonVerifiedEmails (user_id, email, token)")
    db_manager.db_execute_commit("CREATE TABLE ToReVerify (user_id, role_id)")
    db_manager.db_execute_commit("CREATE TABLE Roles (guild_id, role_id, email_suffix)")
...
`{TEST_EMAIL_DOMAIN}` for `@testRole`"""
    ...
    db_manager.db_execute_commit("DELETE FROM Roles WHERE guild_id=1234")
...
async def test_member_join_already_verified(bot):
    ...
    db_manager.db_execute_commit(f"INSERT INTO VerifiedEmails VALUES (999, 'egg@{TEST_EMAIL_DOMAIN}')")
    db_manager.db_execute_commit(f"INSERT INTO Roles VALUES (1234, 555, '{TEST_EMAIL_DOMAIN}')")
...
`{TEST_EMAIL_DOMAIN}` for `@testRole`"""
    ...
    db_manager.db_execute_commit("DELETE FROM VerifiedEmails WHERE user_id=999")
    db_manager.db_execute_commit("DELETE FROM Roles WHERE guild_id=1234")
...
async def test_enable_verification():
    ...
    entry = db_manager.db_execute_select("SELECT * FROM Roles WHERE guild_id=? AND role_id=?",
                                         (guild.id, role.id))
    assert entry
    db_manager.db_execute_commit(f"DELETE FROM Roles WHERE guild_id={guild.id}")
...
async def test_verify():
    ...
    entry = db_manager.db_execute_select(f"SELECT * FROM NonVerifiedEmails WHERE user_id={member.id} AND email='{TEST_EMAIL}'")
    assert entry
...
async def test_confirm():
    ...
    role = dpytest.back.make_role("testRole", guild, id_num=555)
    db_manager.db_execute_commit(f"INSERT INTO Roles VALUES ({guild.id}, 555, 'egg.com')")
    db_manager.db_execute_commit(f"INSERT INTO NonVerifiedEmails VALUES ({member.id}, 'test@egg.com', 'testtoken')")
    dm = await member.create_dm()
    await dpytest.message(KoalaBot.COMMAND_PREFIX + "confirm testtoken", dm)
    verified = db_manager.db_execute_select(f"SELECT * FROM VerifiedEmails WHERE user_id={member.id} AND email='test@egg.com'")
    exists = db_manager.db_execute_select(f"SELECT * FROM NonVerifiedEmails WHERE user_id={member.id} and email='test@egg.com'")
    assert verified
    assert not exists
    ...
    db_manager.db_execute_commit(f"DELETE FROM roles WHERE guild_id={guild.id}")
    db_manager.db_execute_commit(f"DELETE FROM VerifiedEmails WHERE user_id={member.id}")
...
async def test_un_verify():
    ...
    db_manager.db_execute_commit(f"INSERT INTO VerifiedEmails VALUES ({member.id}, 'test@egg.com')")
    ...
    entry = db_manager.db_execute_select(f"SELECT * FROM VerifiedEmails WHERE user_id={member.id} AND email='test@egg.com'")
    assert not entry
...
async def test_get_emails():
    db_manager.db_execute_commit(f"INSERT INTO VerifiedEmails VALUES (123, '{TEST_EMAIL}')")
    ...
    db_manager.db_execute_commit("DELETE FROM VerifiedEmails WHERE user_id=123")
...
async def test_re_verify():
    ...
    assert role not in member.roles
    blacklisted = db_manager.db_execute_select(f"SELECT * FROM ToReVerify WHERE user_id={member.id}")
    assert blacklisted
    ...
    db_manager.db_execute_commit(f"DELETE FROM VerifiedEmails WHERE user_id={member.id}")
    db_manager.db_execute_commit(f"DELETE FROM Roles WHERE guild_id={guild.id}")
    db_manager.db_execute_commit(f"DELETE FROM ToReVerify WHERE user_id={member.id}")
