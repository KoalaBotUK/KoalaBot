...
def test_create_vote(bot: commands.Bot, cog):
    ...
    assert core.start_vote(bot, cog.vote_manager, "Test Vote", author.id, guild.id) == f"Vote titled `Test Vote` created for guild {guild.name}. Use `{koalabot.COMMAND_PREFIX}help vote` to see how to configure it."
...
async def test_vote_already_created(bot: commands.Bot, cog):
    ...
    assert core.start_vote(bot, cog.vote_manager, "Test Vote", author.id, guild.id) == f"You already have an active vote in {guild.name}. Please send that with `{koalabot.COMMAND_PREFIX}vote send` before creating a new one."
...
async def test_vote_already_sent(bot: commands.Bot, cog, session):
    ...
    assert core.start_vote(bot, cog.vote_manager, "Test Vote", author.id, guild.id) == "You already have a vote with title Test Vote sent!"
...
async def test_add_role(bot: commands.Bot, cog, session):
    ...
    await dpytest.message(f"{koalabot.COMMAND_PREFIX}vote create Test Vote")
    assert core.set_roles(bot, cog.vote_manager, author.id, guild.id, role.id, "add") == f"Vote will be sent to those with the {role.name} role"
...
async def test_remove_role(bot: commands.Bot, cog):
    ...
    await dpytest.message(f"{koalabot.COMMAND_PREFIX}vote create Test Vote")
    core.set_roles(bot, cog.vote_manager, author.id, guild.id, role.id, "add")
    
    assert core.set_roles(bot, cog.vote_manager, author.id, guild.id, role.id, "remove") == f"Vote will no longer be sent to those with the {role.name} role"
...
async def test_set_chair(bot: commands.Bot, cog):
    ...
    assert await core.set_chair(bot, cog.vote_manager, author.id, chair.id) == f"Set chair to {chair.name}"
...
async def test_set_chair_no_dms(bot: commands.Bot, cog):
    ...
    with mock.patch('discord.channel.DMChannel', mock.Mock(side_effect=Exception('discord.Forbidden'))):
        with pytest.raises(discord.Forbidden, match="Chair not set as requested user is not accepting direct messages."):
            await core.set_chair(bot, cog.vote_manager, author.id, chair.id)
...
async def test_set_no_chair(bot: commands.Bot, cog):
    ...
    assert await core.set_chair(bot, cog.vote_manager, author.id) == "Results will be sent to the channel vote is closed in"
...
def test_set_channel(bot: commands.Bot, cog):
    ...
    assert core.set_channel(bot, cog.vote_manager, author.id, channel.id) == f"Set target channel to {channel.name}"
...
def test_set_no_channel(bot: commands.Bot, cog):
    ...
    assert core.set_channel(bot, cog.vote_manager, author.id) == "Removed channel restriction on vote"
...
def test_add_option(bot: commands.Bot, cog):
    ...
    assert core.add_option(cog.vote_manager, author.id, "Option 1+Option description") == "Option Option 1 with description Option description added to vote"
...
def test_add_option_wrong_formatting(bot: commands.Bot, cog):
    ...
    assert core.add_option(cog.vote_manager, author.id, "Option 1") == "Example usage: k!vote addOption option title+option description"
...
def test_add_option_too_many(bot: commands.Bot, cog):
    ...
    x = 0
    while (x < 11):
        core.add_option(cog.vote_manager, author.id, "more+options")
        x += 1

    assert core.add_option(cog.vote_manager, author.id, "more options+please?") == "Vote has maximum number of options already (10)"
...
def test_add_option_too_long(bot: commands.Bot, cog):
    ...
    x = 0
    while (x < 5):
        core.add_option(cog.vote_manager, author.id, test_option)
        x += 1

    assert core.add_option(cog.vote_manager, author.id, test_option) == "Option string is too long. The total length of all the vote options cannot be over 1500 characters."
...
def test_remove_option(bot: commands.Bot, cog):
    ...
    core.add_option(cog.vote_manager, author.id, "test+option")

    assert core.remove_option(cog.vote_manager, author.id, 0) == "Option number 0 removed"
...
def test_remove_option_bad(bot: commands.Bot, cog):
    ...
    assert core.remove_option(cog.vote_manager, author.id, 0) == "Option number 0 not found"
...
def test_set_end_time(bot: commands.Bot, cog):
    ...
    assert core.set_end_time(cog.vote_manager, author.id, "2222-12-30 13:30") == "Vote set to end at 2222-12-30 13:30:00 UTC"
...
def test_set_impossible_end_time(bot: commands.Bot, cog):
    ...
    assert core.set_end_time(cog.vote_manager, author.id, "2020-01-15 12:50") == "You can't set a vote to end in the past"
...
def test_preview(bot: commands.Bot, cog):
    ...
    prev = core.preview(cog.vote_manager, author.id)
...
async def test_cancel_sent_vote(bot: commands.Bot, cog):
    ...
    author: discord.Member = guild.members[0]
    core.start_vote(bot, cog.vote_manager, "Test Vote", author.id, guild.id)
    core.add_option(cog.vote_manager, author.id, "Option 1+Option description")
    core.add_option(cog.vote_manager, author.id, "Option 2+Option description2")

    await core.send_vote(bot, cog.vote_manager, author.id, guild.id)
...
async def test_close_no_chair(bot: commands.Bot, cog):
    ...
    embed = await core.close(bot, cog.vote_manager, author.id, "Test Vote")
...
async def test_close_with_chair(bot: commands.Bot, cog):
    ...
    assert await core.close(bot, cog.vote_manager, author.id, "Test Vote") == f"Sent results to {chair}"
...
async def test_send_vote(bot: commands.Bot, cog):
    ...
    assert await core.send_vote(bot, cog.vote_manager, author.id, guild.id) == "Sent vote to 1 users"
...
async def test_send_vote_bad_options(bot: commands.Bot, cog):
    ...
    # no options
    assert await core.send_vote(bot, cog.vote_manager, author.id, guild.id) == "Please add more than 1 option to vote for"
...
async def test_get_results(bot: commands.Bot, cog):
    ...
    embed = await core.results(bot, cog.vote_manager, author.id, "Test Vote")
    assert embed.title == "Test Vote Results:"
    assert embed.fields[0].name == "Option 1"
    assert embed.fields[1].name == "Option 2"
...
async def test_results_vote_not_sent(bot: commands.Bot, cog):
    ...
    assert await core.results(bot, cog.vote_manager, author.id, "Test Vote") == "That vote has not been sent yet. Please send it to your audience with k!vote send Test Vote"
...
async def test_results_invalid_vote(bot: commands.Bot, cog):
    ...
    with pytest.raises(ValueError, match=f"invalid is not a valid vote title for user {author.name}"):
        await core.results(bot, cog.vote_manager, author.id, "invalid")