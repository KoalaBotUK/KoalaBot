...
class ReactForRoleDBManager:
    ...
    def remove_rfr_message(guild_id: int, channel_id: int, message_id: int, session: sqlalchemy.orm.Session):
        ...
        :return:
        """
        emoji_role_id = ReactForRoleDBManager.get_rfr_message(guild_id, channel_id, message_id)
        if not emoji_role_id:
            return
        else:
            ReactForRoleDBManager.remove_rfr_message_emoji_roles(emoji_role_id[3])
    ...
    def get_guild_rfr_roles(guild_id: int) -> List[int]:
        ...
        with session_manager() as session:
            ...
            for rfr_message in rfr_messages:
                roles: List[Tuple[int, str, int]] = ReactForRoleDBManager.get_rfr_message_emoji_roles(rfr_message.emoji_role_id)
                if not roles:
                    continue
