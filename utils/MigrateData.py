# Own modules
import KoalaBot
# Variables
from utils.KoalaDBManager import KoalaDBManager

class MigrateData:

    def __init__(self):
        self.migrate_database_manager = MigrateDataDBManager(KoalaBot.database_manager)
        self.migrate_database_manager.execute_update()

class MigrateDataDBManager:

    def __init__(self, database_manager: KoalaDBManager.KoalaDBManager):
        self.database_manager = database_manager

    def execute_update(self):
        self.remake_guids()
        self.remake_guild_extensions()
        self.remake_guild_welcome_messages()
        self.remake_votes()
        self.remake_vote_sent()
        self.remake_vote_options()
        self.remake_vote_target_roles()
        self.remake_verified_emails()
        self.remake_not_verified_emails()
        self.remake_role_table()
        self.remake_to_re_verify()
        self.remake_twitch_alerts()
        self.remake_user_in_twitch_alert()
        self.remake_team_in_twitch_alert()
        self.remake_user_in_twitch_team()
        self.remake_text_filter()
        self.remake_text_filter_moderation()
        self.remake_text_filter_ignore_list()
        self.remake_guild_rf_messages()
        self.remake_rfr_message_emoji_roles()
        self.remake_guild_rfr_required_roles()
        self.remake_guild_colour_change_permissions()
        self.remake_guild_invalid_custom_colour_roles()
        self.remake_guild_usage()

    def remake_guilds(self):
        sql_create_guilds_table = """
        CREATE TABLE IF NOT EXISTS Guilds (
        guild_id text NOT NULL,
        subscription integer NOT NULL DEFAULT=0
        PRIMARY KEY (guild_id)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM GuildExtensions;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS Guilds;""")
        self.database_manager.de_execute_commit(sql_create_guilds_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO  (guild_id, subscription) VALUES (?, ?);""",
                args=[i[0], 0])

    def remake_guild_extensions(self):
        sql_create_guild_extensions_table = """
        CREATE TABLE IF NOT EXISTS GuildExtensions (
        extension_id text NOT NULL,
        guild_id text NOT NULL,
        PRIMARY KEY (extension_id,guild_id),
        CONSTRAINT fk_extensions
            FOREIGN KEY (extension_id) 
            REFERENCES KoalaExtensions (extension_id)
            ON DELETE CASCADE 

            FOREIGN KEY (guild_id)
            REFERENCES Guilds (guild_id)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM GuildExtensions;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS GuildExtensions;""")
        self.database_manager.de_execute_commit(sql_create_guild_extensions_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO GuildExtensions (extension_id, guild_id) VALUES (?, ?);""",
                args=list(i))

    def remake_guild_welcome_messages(self):
        sql_create_guild_welcome_messages_table = """
        CREATE TABLE IF NOT EXISTS GuildWelcomeMessages (
        guild_id text NOT NULL PRIMARY KEY,
        welcome_message text,
        PRIMARY KEY guild_id,
        FOREIGN KEY guild_id REFERENCES Guilds (guild_id)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM GuildWelcomeMessages;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS GuildWelcomeMessages;""")
        self.database_manager.de_execute_commit(sql_create_guild_welcome_messages_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO GuildWelcomeMessages (guild_id, welcome_message) VALUES (?, ?);""",
                args=list(i))

    def remake_votes(self):
        vote_table = """
        CREATE TABLE IF NOT EXISTS Votes (
        vote_id integer NOT NULL,
        author_id text NOT NULL,
        guild_id text NOT NULL,
        title text NOT NULL,
        chair_id text,
        voice_id text,
        end_time float,
        PRIMARY KEY author_id,
        FOREIGN KEY guild_id REFERENCES Guilds (guild_id)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM Votes;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS Votes;""")
        self.database_manager.de_execute_commit(vote_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO  (vote_id, author_id, guild_id, title, chair_id, voice_id, end_time) VALUES (?, ?, ?, ?, ?, ?, ?);""",
                args=list(i))

    def remake_vote_sent(self):
        delivered_table = """
        CREATE TABLE IF NOT EXISTS VoteSent (
        vote_id text NOT NULL,
        vote_receiver_id text NOT NULL,
        vote_receiver_message text NOT NULL,
        PRIMARY KEY vote_id,
        FOREIGN KEY vote_id REFERNCES Votes (vote_id)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM VoteSent;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS VoteSent;""")
        self.database_manager.de_execute_commit(delivered_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO VoteSent (vote_id, vote_receiver_id, vote_receiver_message) VALUES (?, ?, ?);""",
                args=list(i))

    def remake_vote_options(self):
        option_table = """
        CREATE TABLE IF NOT EXISTS VoteOptions (
        vote_id text NOT NULL,
        opt_id text NOT NULL,
        option_title text NOT NULL,
        option_desc text NOT NULL,
        PRIMARY KEY vote_id,
        FOREIGN KEY vote_id REFERENCES Votes (vote_id)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM VoteOptions;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS VoteOptions;""")
        self.database_manager.de_execute_commit(option_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO VoteOptions (vote_id, opt_id, option_title, option_desc) VALUES (?, ?, ?, ?);""",
                args=list(i))

    def remake_vote_target_roles(self):
        role_table = """
        CREATE TABLE IF NOT EXISTS VoteTargetRoles (
        vote_id text NOT NULL,
        role_id text NOT NULL,
        PRIMARY KEY vote_id,
        FOREIGN KEY vote_id REFERENCES Votes (vote_id)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM VoteTargetRoles;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS VoteTargetRoles;""")
        self.database_manager.de_execute_commit(role_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO VoteTargetRoles (vote_id, role_id) VALUES (?, ?);""",
                args=list(i))

    def remake_verified_emails(self):
        verified_table = """
        CREATE TABLE IF NOT EXISTS VerifiedEmails (
        user_id text NOT NULL,
        email text NOT NULL,
        PRIMARY KEY (user_id, email)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM VerifiedEmails;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS VerifiedEmails;""")
        self.database_manager.de_execute_commit(verified_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO VerifiedEmails (user_id, email) VALUES (?, ?);""",
                args=list(i))

    def remake_not_verified_emails(self):
        non_verified_table = """
        CREATE TABLE IF NOT EXISTS NonVerifiedEmails (
        user_id text NOT NULL,
        email text NOT NULL,
        token text NOT NULL,
        PRIMARY KEY (token)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM NonVerifiedEmails;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS NonVerifiedEmails;""")
        self.database_manager.de_execute_commit(non_verified_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO NonVerifiedEmails (user_id, email, token) VALUES (?, ?, ?);""",
                args=list(i))

    def remake_role_table(self):
        role_table = """
        CREATE TABLE IF NOT EXISTS Roles (
        guild_id text NOT NULL,
        role_id text NOT NULL,
        email_suffix text NOT NULL,
        PRIMARY KEY (guild_id, role_id, email_suffix),
        FOREIGN KEY (guild_id) REFERENCES Guilds (guild_id)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM Roles;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS Roles;""")
        self.database_manager.de_execute_commit(role_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO Roles (guild_id, role_id, email_suffix) VALUES (?, ?, ?);""",
                args=list(i))

    def remake_to_re_verify(self):
        re_verify_table = """
        CREATE TABLE IF NOT EXISTS ToReVerify (
        user_id text NOT NULL,
        role_id text NOT NULL,
        PRIMARY KEY (user_id, role_id)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM ToReVerify;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS ToReVerify;""")
        self.database_manager.de_execute_commit(re_verify_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO ToReVerify (user_id, role_id) VALUES (?, ?);""",
                args=list(i))

    def remake_twitch_alerts(self):
        sql_create_twitch_alerts_table = """
        CREATE TABLE IF NOT EXISTS TwitchAlerts (
        guild_id text NOT NULL,
        channel_id text NOT NULL,
        default_message text NOT NULL,
        PRIMARY KEY (guild_id, channel_id),
        CONSTRAINT fk_guild
            FOREIGN KEY (guild_id) 
            REFERENCES Guilds (guild_id)
            ON DELETE CASCADE 
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM TwitchAlerts;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS TwitchAlerts;""")
        self.database_manager.de_execute_commit(sql_create_twitch_alerts_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO TextTable (guild_id, channel_id, default_message) VALUES (?, ?, ?);""",
                args=list(i))

    def remake_user_in_twitch_alert(self):
        sql_create_user_in_twitch_alert_table = """
        CREATE TABLE IF NOT EXISTS UserInTwitchAlert (
        channel_id text NOT NULL,
        twitch_username text NOT NULL,
        custom_message text,
        message_id text,
        PRIMARY KEY (channel_id, twitch_username),
        CONSTRAINT fk_channel
            FOREIGN KEY (channel_id) 
            REFERENCES TwitchAlerts (channel_id)
            ON DELETE CASCADE 
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM UserInTwitchAlert;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS UserInTwitchAlert;""")
        self.database_manager.de_execute_commit(sql_create_user_in_twitch_alert_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO UserInTwitchAlert (chanel_id, twitch_username, custom_message, message_id) VALUES (?, ?, ? ,?);""",
                args=list(i))

    def remake_team_in_twitch_alert(self):
        sql_create_team_in_twitch_alert_table = """
        CREATE TABLE IF NOT EXISTS TeamInTwitchAlert (
        team_twitch_alert_id integer PRIMARY KEY AUTOINCREMENT, 
        channel_id text NOT NULL,
        twitch_team_name text NOT NULL,
        custom_message text,
        CONSTRAINT fk_channel
            FOREIGN KEY (channel_id) 
            REFERENCES TwitchAlerts (channel_id)
            ON DELETE CASCADE 
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM TeamInTwitchAlert;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS TeamInTwitchAlert;""")
        self.database_manager.de_execute_commit(sql_create_team_in_twitch_alert_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO TeamInTwitchAlert (team_twitch_alert_id, channel_id, twitch_team_name, custom_message) VALUES (?, ?, ?, ?);""",
                args=list(i))

    def remake_user_in_twitch_team(self):
        sql_create_user_in_twitch_team_table = """
        CREATE TABLE IF NOT EXISTS UserInTwitchTeam (
        team_twitch_alert_id integer NOT NULL,
        twitch_username text NOT NULL,
        message_id text,
        PRIMARY KEY (team_twitch_alert_id, twitch_username),
        CONSTRAINT fk_twitch_team_alert
            FOREIGN KEY (team_twitch_alert_id) 
            REFERENCES TeamInTwitchAlert (team_twitch_alert_id)
            ON DELETE CASCADE 
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM UserInTwitchTeam;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS UserInTwitchTeam;""")
        self.database_manager.de_execute_commit(sql_create_user_in_twitch_team_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO UserInTwitchTeam (team_twitch_alert_id, twitch_username, meaage_id) VALUES (?, ?, ?);""",
                args=list(i))

    def remake_text_filter(self):
        sql_create_text_filter_table = """
        CREATE TABLE IF NOT EXISTS TextFilter (
        filtered_text_id text NOT NULL,
        guild_id text NOT NULL,
        filtered_text text NOT NULL,
        filter_type text NOT NULL,
        is_regex boolean NOT NULL,
        PRIMARY KEY (filtered_text_id),
        FOREIGN KEY guild_id REFERENCES Guilds (guild_id)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM TextFilter;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS TextFilter;""")
        self.database_manager.de_execute_commit(sql_create_text_filter_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO TextFilter (filtered_text_id, guild_id, filtered_text, filter_type, is_regex) VALUES (?, ?, ?, ?, ?);""",
                args=list(i))

    def remake_text_filter_moderation(self):
        sql_create_mod_table = """
        CREATE TABLE IF NOT EXISTS TextFilterModeration (
        channel_id text NOT NULL,
        guild_id text NOT NULL,
        PRIMARY KEY (channel_id),
        FOREIGN KEY guild_id REFERENCES Guilds (guild_id)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM TextFilterModeration;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS TextFilterModeration;""")
        self.database_manager.de_execute_commit(sql_create_mod_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO TextFilterModeration (chanel_id, guild_id) VALUES (?, ?);""",
                args=list(i))

    def remake_text_filter_ignore_list(self):
        sql_create_ignore_list_table = """
        CREATE TABLE IF NOT EXISTS TextFilterIgnoreList (
        ignore_id text NOT NULL,
        guild_id text NOT NULL,
        ignore_type text NOT NULL,
        ignore text NOT NULL,
        PRIMARY KEY (ignore_id),
        FOREIGN KEY guild_id REFERENCES Guilds (guild_id)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM TextFilterIgnoreList;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS TextFilterIgnoreList;""")
        self.database_manager.de_execute_commit(sql_create_ignore_list_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO TextFilterIgnoreList (ignore_id, guild_id, ignore_type, ignore) VALUES (?, ?, ?, ?);""",
                args=list(i))

    def remake_guild_rf_messages(self):
        sql_create_guild_rfr_message_ids_table = """
        CREATE TABLE IF NOT EXISTS GuildRFRMessages (
        guild_id text NOT NULL,
        channel_id text NOT NULL,
        message_id text NOT NULL,
        emoji_role_id text,
        PRIMARY KEY (emoji_role_id),
        FOREIGN KEY (guild_id) REFERENCES Guilds (guild_id),
        UNIQUE (guild_id, channel_id, message_id)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM GuildRFRMessages;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS GuildRFRMessages;""")
        self.database_manager.de_execute_commit(sql_create_guild_rfr_message_ids_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO GuildRFRMessages (guild_id, channel_id, message_id, emoji_role_id) VALUES (?, ?, ?, ?);""",
                args=list(i))

    def remake_rfr_message_emoji_roles(self):
        sql_create_rfr_message_emoji_roles_table = """
        CREATE TABLE IF NOT EXISTS RFRMessageEmojiRoles (
        emoji_role_id text NOT NULL,
        emoji_raw text NOT NULL,
        role_id text NOT NULL,
        PRIMARY KEY (emoji_role_id, emoji_raw, role_id),
        FOREIGN KEY (emoji_role_id) REFERENCES GuildRFRMessages(emoji_role_id),
        UNIQUE (emoji_role_id, emoji_raw),
        UNIQUE  (emoji_role_id, role_id)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM RFRMessageEmojiRoles;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS RFRMessageEmojiRoles;""")
        self.database_manager.de_execute_commit(sql_create_rfr_message_emoji_roles_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO RFRMessageEmojiRoles (emoji_role_id, emoji_raw, role_id) VALUES (?, ?, ?);""",
                args=list(i))

    def remake_guild_rfr_required_roles(self):
        sql_create_rfr_required_roles_table = """
        CREATE TABLE IF NOT EXISTS GuildRFRRequiredRoles (
        guild_id text NOT NULL,
        role_id text NOT NULL,
        PRIMARY KEY (guild_id, role_id),
        FOREIGN KEY (guild_id) REFERENCES Guilds (guild_id),
        UNIQUE (guild_id, role_id)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM GuildRFRRequiredRoles;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS GuildRFRRequiredRoles;""")
        self.database_manager.de_execute_commit(sql_create_rfr_required_roles_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO GuildRFRRequiredRoles (guild_id, role_id) VALUES (?, ?, ?);""",
                args=list(i))

    def remake_guild_colour_change_permissions(self):
        sql_create_guild_colour_change_permissions_table = """
        CREATE TABLE IF NOT EXISTS GuildColourChangePermissions (
        guild_id text NOT NULL,
        role_id integer NOT NULL,
        PRIMARY KEY (guild_id, role_id),
        FOREIGN KEY (guild_id) REFERENCES Guilds (guild_id)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM GuildColourChangePermissions;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS GuildColourChangePermissions;""")
        self.database_manager.de_execute_commit(sql_create_guild_colour_change_permissions_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO GuildColourChangePermissions (guild_id, role_id) VALUES (?, ?);""",
                args=list(i))

    def remake_guild_invalid_custom_colour_roles(self):
        sql_create_guild_colour_change_invalid_colours_table = """
        CREATE TABLE IF NOT EXISTS GuildInvalidCustomColourRoles (
        guild_id text NOT NULL,
        role_id integer NOT NULL,
        PRIMARY KEY (guild_id, role_id),
        FOREIGN KEY (guild_id) REFERENCES Guilds (guild_id)
        );"""

        data = self.database_manager.de_execute_select("""SELECT * FROM GuildInvalidCustomColourRoles;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS GuildInvalidCustomColourRoles;""")
        self.database_manager.de_execute_commit(sql_create_guild_colour_change_invalid_colours_table)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO GuildInvalidCustomColourRoles (guild_id, role_id) VALUES (?, ?);""",
                args=list(i))

    def remake_guild_usage(self):
        sql_create_usage_tables = """
        CREATE TABLE IF NOT EXISTS GuildUsage (
        guild_id text NOT NULL,
        last_message_epoch_time text NOT NULL,
        PRIMARY KEY (guild_id),
        FOREIGN KEY (guild_id) REFERENCES Guilds (guild_id)
        );
        """

        data = self.database_manager.de_execute_select("""SELECT * FROM GuildUsage;""")
        self.database_manager.de_execute_commit("""DROP TABLE IF EXISTS GuildUsage;""")
        self.database_manager.de_execute_commit(sql_create_usage_tables)
        for i in data:
            self.database_manager.de_execute_commit(
                """INSERT INTO GuildUsage (guild_id, last_message_epoch_time) VALUES (?, ?);""",
                args=list(i))
