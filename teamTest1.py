...
def pick_teams(player_list):
    return pick_teams_compa_list(player_list, [], player_list)
...
def create_value_attitude(first_player, second_player):
    ...
    if rank_difference == 0:
        return 0
    if second_player[6] | first_player[6]:
        return -1
...
def pick_teams_compa_list(player_list, input_teams, untouched_player_list):
    team = []
    teams = input_teams
    ...
    while len(team) < (team_size + 1):
        ...
        next_player = get_next_player(highest_player_compat_list, remove_subs(player_list, get_sub_list(player_list)))
    ...
    if len(player_list) >= team_size:
        pick_teams_compa_list(player_list, teams, untouched_player_list)
    return assign_subs(convert_teams(teams), get_sub_list(untouched_player_list))
